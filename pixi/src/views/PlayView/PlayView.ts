...
export class PlayViewClass {
    ...
    private updateSpawn(dt: number) {
        ...
        if (this.spawnTimer >= this.nextSpawnInterval) {
            ...
            const minInt = Math.max(this.minSpawnInterval, this.minSpawnInterval / speedFactor);
            const maxInt = Math.max(this.maxSpawnInterval, this.maxSpawnInterval / speedFactor);

            this.nextSpawnInterval = Math.floor(Math.random() * (maxInt - minInt + 1)) + minInt;
        }
    }
