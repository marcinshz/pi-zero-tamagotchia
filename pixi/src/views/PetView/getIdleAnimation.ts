...
export function getIdleAnimation(): Animations {
    ...
    const defaultIdleAnimations: Animations[] = [Animations.IDLE, Animations.DANCE, Animations.HOBBY]

    if (!possibleIdleAnimations.length) return defaultIdleAnimations[Math.floor(Math.random() * defaultIdleAnimations.length)];
